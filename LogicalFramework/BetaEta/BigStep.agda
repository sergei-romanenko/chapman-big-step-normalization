...
mutual
  data ev_&_⇓_ : ∀ {Γ Δ σ} → Tm Δ σ → (vs : Env Γ Δ) → 
                 Val Γ (σ [ embˢ vs ]⁺) → Set where
    evcoe : ∀ {Γ Δ  Δ'}{σ : Ty Δ}{σ'}{t : Tm Δ' σ'}{p : σ' ≡⁺ σ}
            {vs : Env Γ Δ}{v : Val Γ _} → 
            ev t & coevˢ vs reflˠ (symˠ (fog⁺ p)) ⇓ v →
            ev coe t p & vs ⇓ coev v (cong⁺ p (symˢ (cohvˢ vs reflˠ _)))
    ev[] : ∀ {B Γ Δ σ}{t : Tm Δ σ}{ts : Sub Γ Δ}{vs : Env B Γ}
           {vs' : Env B Δ}(p : evˢ ts & vs ⇓ vs') → 
           {v : Val B _} → ev t & vs' ⇓ v → 
           ev t [ ts ] & vs ⇓ coev v (trans⁺ (cong⁺ refl⁺
                                                    (symˢ (comevˢ ts vs p)))
                                             (sym⁺ assoc⁺))
    evtop : ∀ {Γ Δ σ}{vs : Env Γ Δ}{v : Val Γ (σ [ embˢ vs ]⁺)} →
            ev top & vs << v ⇓ coev v (trans⁺ (cong⁺ refl⁺ (symˢ pop<))
                                              (sym⁺ assoc⁺))
    evλ : ∀ {Γ Δ σ τ}{t : Tm (Δ , σ) τ}{vs : Env Γ Δ} →
          ev λt t & vs ⇓ λv t vs
    evapp : ∀ {Γ Δ σ τ}{t : Tm Δ (Π σ τ)}{vs : Env Γ Δ}
            {v : Val Γ _}{v' : Val Γ _} → ev t & vs ⇓ v' → 
            {v'' : Val Γ _} → (vapp v' & refl⁺ & v ⇓ v'') → 
            ev app t & vs << v ⇓ v'' 

  data evˢ_&_⇓ : ∀ {Γ Δ Σ} → Sub Δ Σ → Env Γ Δ → Env Γ Σ → Set where
    evˢcoeˢ : ∀ {Γ Δ Δ' Σ Σ'}{ts : Sub Δ' Σ'}{p : Δ' ≡ˠ Δ}{q : Σ' ≡ˠ Σ}
              {vs : Env Γ Δ}{vs' : Env Γ Σ'} → 
              evˢ ts & coevˢ vs reflˠ (symˠ p) ⇓ vs' →
              evˢ coeˢ ts p q & vs ⇓ (coevˢ vs' reflˠ q)
    evˢ• : ∀ {B Γ Δ Σ}{ts : Sub Δ Σ}{us : Sub Γ Δ}{vs : Env B Γ}
           {vs' : Env B Δ} → evˢ us & vs ⇓ vs' →
           {vs'' : Env B Σ} → evˢ ts & vs' ⇓ vs'' →
           evˢ ts • us & vs ⇓ vs''
    evˢid : ∀ {Γ Δ}{vs : Env Γ Δ} → evˢ id & vs ⇓ vs           
    evˢpop : ∀ {Γ Δ}{σ : Ty Δ}{vs : Env Γ Δ}
             {v : Val Γ (σ [ embˢ vs ]⁺)} → 
             evˢ (pop σ) & (vs << v) ⇓ vs
    evˢ< : ∀ {B Γ Δ σ}{ts : Sub Γ Δ}{t : Tm Γ (σ [ ts ]⁺)}{vs : Env B Γ}
           {vs' : Env B Δ}(p : evˢ ts & vs ⇓ vs') →
           {v : Val B _} → ev t & vs ⇓ v →
  ...
  data vapp_&_&_⇓_ : ∀ {Γ Γ' Δ σ τ ρ}{ts : Sub Γ Δ}(v : Val Γ' ρ) →
                     ρ ≡⁺ Π σ τ [ ts ]⁺ → (v' : Val Γ (σ [ ts ]⁺)) →
                     (v'' : Val Γ (τ [ ts < emb v' ]⁺)) → Set where
    vappλv : ∀ {Γ Γ' Δ Δ' σ τ σ' τ'}{ts : Sub Γ' Δ'}{t : Tm (Δ , σ) τ}
  ...
  comevˢ : ∀ {Γ Δ Σ}(ts : Sub Δ Σ)(vs : Env Γ Δ){vs' : Env Γ Σ} →
...
data ev⁺_&_⇓_ : ∀ {Γ Δ} → Ty Δ → Env Γ Δ → VTy Γ → Set where
  ev⁺coe⁺ : ∀ {Γ Δ Δ'}{σ : Ty Δ'}{p : Δ' ≡ˠ Δ}{vs : Env Γ Δ}
            {σ' : VTy Γ} → ev⁺ σ & coevˢ vs reflˠ (symˠ p) ⇓ σ' →
            ev⁺ coe⁺ σ p & vs ⇓ σ'
  ev⁺[] : ∀ {B Γ Δ}{σ : Ty Δ}{ts : Sub Γ Δ}{vs : Env B Γ}
          {vs' : Env B Δ} → evˢ ts & vs ⇓ vs' →
          {σ' : VTy B} → ev⁺ σ & vs' ⇓ σ' → ev⁺ σ [ ts ]⁺ & vs ⇓ σ'
  ev⁺U : ∀ {Γ Δ}{vs : Env Γ Δ} → ev⁺ U & vs ⇓ VU
  ev⁺El : ∀ {Γ Δ}{σ : Tm Δ U}{vs : Env Γ Δ}
          {σ' : Val Γ (U [ embˢ vs ]⁺)} → ev σ & vs ⇓ σ' → 
          ev⁺ El σ & vs ⇓ VEl (coev σ' U[])
          
  ev⁺Π : ∀ {Γ Δ σ τ}{vs : Env Γ Δ} → ev⁺ Π σ τ & vs ⇓ VΠ σ τ vs

comev⁺ : ∀ {Γ Δ}{σ : Ty Δ}{vs : Env Γ Δ}{σ' : VTy Γ} → ev⁺ σ & vs ⇓ σ' →
         σ [ embˢ vs ]⁺ ≡⁺ emb⁺ σ'
